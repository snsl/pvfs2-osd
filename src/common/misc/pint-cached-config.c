...
const struct handle_lookup_entry* find_handle_lookup_entry(
                                                     PVFS_handle handle,
                                                     PVFS_fs_id fsid);
                                     ...
                                     int is_directory)
{
    ...
    /* figure out if we want or do not want an OSD based on
     * type settings and if a directory server was requested
     * (from mkdir).
     */
    want_osd = 0;  /* 0 == give me a pvfs */
    switch (config->osd_type) {
        case OSD_NONE:
            if (config->osd_dir_type == OSD_DIR_PVFS)
                want_osd = 0;
            else
                want_osd = is_directory;
            break;
        case OSD_DATAFILE:
            if (config->osd_dir_type == OSD_DIR_PVFS)
                want_osd = 0;
        ...
        case OSD_MDFILE:
            if (config->osd_dir_type == OSD_DIR_PVFS)
                want_osd = !is_directory;  /* never called */
            else
                want_osd = 1;
            break;
    }
    ...
    while(randsrv > 0)
    {
        ...
        /* first jitter across all, regardless of acceptability, then
         * keep looping until find one that matches */
        if (randsrv == 0)
        {
            if (want_osd ^ alias_is_osd(cur_mapping->alias_mapping->bmi_address))
                continue;
        }
                                                     ...
                                                     PVFS_fs_id fsid)
{
    ...
    if(PINT_handle_in_extent(
                    ...
                    handle))
    {
        if(!strncmp(cur_config_cache->handle_lookup_table[table_index].server_name, "osd", 3))
        {
            /* osd address might be wrong, since the handle lookup table doesn't call BMI_addr_lookup for osd server */
            BMI_addr_lookup(&cur_config_cache->handle_lookup_table[table_index].server_addr, cur_config_cache->handle_lookup_table[table_index].server_name);
        }
