...
typedef struct server_configuration_s
{
    ...
    enum {
        OSD_NONE,      /* stock pvfs */
        OSD_DATAFILE,  /* datafiles stored on OSDs */
        OSD_METAFILE,  /* df and mf, with mf existing as attrs; but not dirs */
        OSD_MDFILE,    /* df and mf, with mf existing as attrs on first df */
    } osd_type;

    enum {
        OSD_DIR_PVFS,   /* stock pvfs */
        OSD_DIR_ATTR4,  /* dirs in attrs, 4-step insert */
        OSD_DIR_ATTR1,  /* dirs in attrs, 1-step insert */
        OSD_DIR_OBJ,    /* dirs in objs in collections */
    } osd_dir_type;

    enum {
        NO_POST_CREATE,		/* create the object first and then write to it */     
        POST_CREATE,		/* delay the creates, create and write at the same time */  
    } post_create;
