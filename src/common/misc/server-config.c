...
static const configoption_t options[] =
{
    ...
    *  is specified!
    ...
    /*
     * Specifies if the IO servers and/or metadata servers are OSDs.
     * OSDDirType says how directories are handled.
     */
    { "OSDType", ARG_STR, get_osd_type, NULL, CTX_DEFAULTS, "none" },
    { "OSDdirType", ARG_STR, get_osd_dir_type, NULL, CTX_DEFAULTS, "pvfs" },

    /*
     * Specifies if postcreating is enabled or not
     */
    { "PostCreate", ARG_STR, get_create_type, NULL, CTX_DEFAULTS, "no" },
...
static DOTCONF_CB(get_create_type)
{
    ...
    if (!strcmp(str, "no"))
        config->post_create = NO_POST_CREATE;
...
static DOTCONF_CB(get_member_attr_type)
{
    ...
    if (!strcmp(str, "no"))
        config->member_attr = NO_MEMBER_ATTR;
    else if (!strcmp(str, "yes"))
...
static DOTCONF_CB(get_osd_type)
{
    ...
    if (!strcmp(str, "none"))
        config->osd_type = OSD_NONE;
    else if (!strcmp(str, "datafile"))
        config->osd_type = OSD_DATAFILE;
    else if (!strcmp(str, "metafile"))
        config->osd_type = OSD_METAFILE;
    else if (!strcmp(str, "mdfile"))
    {
        config->osd_type = OSD_MDFILE;
    }
    else
        ret = "get_osd_type: unknown FileSystem OSDType option\n";
    return ret;
}
...
static DOTCONF_CB(get_osd_dir_type)
{
    ...
    if (!strcmp(str, "pvfs"))
        config->osd_dir_type = OSD_DIR_PVFS;
    else if (!strcmp(str, "attr4"))
        config->osd_dir_type = OSD_DIR_ATTR4;
    else if (!strcmp(str, "attr1"))
        config->osd_dir_type = OSD_DIR_ATTR1;
    else if (!strcmp(str, "obj"))
        config->osd_dir_type = OSD_DIR_OBJ;
    else
        ret = "get_osd_dir_type: unknown FileSystem OSDDirType option\n";
    return ret;
}
    ...
    if (config_s && config_s->host_id && fs)
    {
        ...
        if (my_alias)
        {
            ...
            while(cur)
            {
                ...
                if (strcmp(cur_h_mapping->alias_mapping->host_alias,
                           my_alias) == 0 || !strncmp(cur_h_mapping->alias_mapping->bmi_address, "osd", 3))
                {
                    ret = PINT_merge_handle_range_strs(ret, cur_h_mapping->handle_range);
                }
                cur = PINT_llist_next(cur);
            }
        }
