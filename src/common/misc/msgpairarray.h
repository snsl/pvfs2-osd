...
extern struct PINT_state_machine_s pvfs2_osd_msgpairarray_sm;
...
typedef struct PINT_sm_msgpair_state_s
{
    ...
    /* request when talking to an OSD server */
    struct osd_command osd_command;

    /* point to things that may need freed during clean up */
    void *osd_iov;
    void *osd_sgl;
    ...
    */
    ...
    /*
     * Process the entire msgpairarray, but suppress submitting or completing
     * this one.  Useful when otherwise the array would have to be realloced
     * and shuffled around.  Only implemented in OSD msgpairarray.
     */
    int suppress;
