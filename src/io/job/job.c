...
/*
 * Post a job that does an OSD command submit using bmi_osd.
 */
int job_bmi_osd_submit(PVFS_BMI_addr_t addr, struct osd_command *command,
                       job_aint status_user_tag, job_context_id context_id,
                       void *job_user_ptr, int timeout_sec)
{
    struct job_desc *jd;
    int ret;
    ...
    /* Aribtary data, returned on job completion, like index into msgpair
     * array. */
    jd->status_user_tag = status_user_tag;

    /* This is the sm_p that is returned back to PINT_client_state_machine_test
     * so that it knows what machine to progress. */
    jd->job_user_ptr = job_user_ptr;

    /* BMI_testcontext calls this when done. */
    jd->bmi_callback.fn = bmi_thread_mgr_callback;
    jd->bmi_callback.data = jd;

    /* Used by completion_query_context. */
    jd->context_id = context_id;

    ret = bmi_osd_submit_command(addr, command, &jd->bmi_callback);
    if (ret < 0) {
        dealloc_job_desc(jd);
        return ret;
    }
